...
def get_game_ids(date_obj):
    ...
    if not events:
        print(f"⚠️ No events returned for {date_str}")
    else:
        print(f"✅ {len(events)} games found for {date_str}")
        for e in events:
    ...
    for e in events:
        try:
            ...
            print(f"⚠️ Error parsing event object: {e} → {ex}")
...
def scrape_and_append_pitchers_only():
    ...
    archive_file = os.path.join(archive_dir, f"boxscores_pitcher_append_{timestamp}.csv")
...
if __name__ == "__main__":
    print("🚀 Appending pitcher data from today -2 through today +2")
