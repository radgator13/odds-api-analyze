...
TARGET_URL = "https://stathead.com/baseball/player-pitching-game-finder.cgi"

# === Fixed download/output directories ===
...
try:
    ...
    print("🌐 Navigated to player pitching finder.")
    ...
    debug("step2_clicked_lastn")

    last_n_input = wait.until(EC.presence_of_element_located((By.NAME, "previous_days")))
    last_n_input.clear()
    last_n_input.send_keys("5")
    print("✍️ Entered 5.")
    debug("step3_entered_5")

    # === Select "Innings Pitched" ===
    stat_dropdown = wait.until(EC.presence_of_element_located((By.XPATH, '//select[contains(@class, "stat_criteria no_chosen")]')))
    Select(stat_dropdown).select_by_visible_text("Innings Pitched")
    print("📊 Selected 'Innings Pitched'.")
    debug("step4_selected_innings_pitched")

    # === Enter 3.5 in IP field ===
    innings_input = wait.until(EC.presence_of_element_located((By.XPATH, '//input[@class="criteria-val" and @name="cval[1]"]')))
    innings_input.clear()
    innings_input.send_keys("3.5")
    print("🔢 Entered 3.5 into Innings Pitched field.")
    debug("step5_entered_ip_3_5")
    ...
    if os.path.exists(CSV_PATH):
        ...
        combined_df = combined_df.drop_duplicates(subset=["Player", "Date", "Team", "IP", "Result"]).reset_index(drop=True)
